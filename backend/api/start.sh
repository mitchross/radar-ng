...
# No uvicorn access log: Caddy already logs every request as JSON, and the
# duplicate line per request cost the single API process CPU under load.
python -m uvicorn backend.api.api.server:app --host 127.0.0.1 --port 8000 --no-access-log 2>&1 | tee /tmp/uvicorn.log &
