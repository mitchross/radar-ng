...
- `src/app/(tabs)/_layout.tsx` — Cumulus 5-tab layout (Home/Nowcast/Radar/Alerts/Settings) per design_handoff_radar_app. Floating translucent dark pill (rgba 10,10,20,0.78). Hidden when active route === "radar" (full-bleed map). AlertsIcon has red badge with useAlerts().features.length. SettingsIcon (gear ring + dot). ~140 tokens.
...
- `src/app/(tabs)/alerts.tsx` — NWS Alerts tab. storm-gradient bg, severity-colored cards (left stripe + pill). Tap → /alert/[id] modal. EmptyState with green "all clear" dot when no active alerts. Refresh button top-right. ~200 tokens.
- `src/app/(tabs)/settings.tsx` — Cumulus Settings tab (replaced modal). clearNight gradient. Sections: violet SELF-HOSTED STACK hero (UPTIME/TILES/CACHE), Stack URL editor (single TextInput + Save btn — source of truth for all endpoints), Data Sources (5 rows: Radar tiles / Forecast / Basemap / Satellite-OFF / NWS Alerts, each with status pill + inline editor revealing derived URL), Docker Stack (7 container rows w/ health dots), Tile Cache, Network (refresh interval select, cellular toggle), Preferences (dark basemap, F/C, palette, opacity+playback sliders), About. ~500 tokens.
...
- `src/components/timeline/TimelineBar.tsx` — Apple-Weather forecast pill (per handoff). VIBRANT WHITE glass card bottom 44, radius 28, shadow. Dark play button, "Precipitation · Now" title + day-of-week/clock, 1h/12h segmented zoom. Playback ticks every 420ms within zoom window (useRef avoids stale closures). 1h window: frames within [now-60m, now+60m]; 12h: full frames. Snaps thumb into window on zoom change. Segmented track (past solid dark / nowcast solid violet / HRRR dashed violet / long-range fainter dashed) computed from frame Unix seconds vs "now" using NOWCAST_MIN=60, HRRR_MIN=360. Dark NOW marker (2px). White thumb via Slider overlay. Axis ticks adapt to zoom: -60/-30/Now/+30/+60 or -1h/Now/+3h/+12h/+24h. ~340 tokens.
