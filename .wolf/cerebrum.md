...
- (2026-04-23) design_handoff_radar_app: went from 3 tabs to 5 (Home/Nowcast/Radar/Alerts/Settings). Settings is no longer a modal — it's a tab. Single stack URL field in Settings is the source of truth; Data Sources list shows derived endpoints (radar/forecast/basemap) computed from serverUrl + SELF_HOSTED paths — the individual rows are read-only, editing goes through the top URL field. Tab bar hides when activeRoute === "radar" for full-bleed map. TimelineBar became a light vibrant-glass "forecast pill" with 1h/12h zoom segmented + 420ms playback looping within window (not the user's playbackSpeed setting — that's orthogonal and still exposed in Preferences for future use).
