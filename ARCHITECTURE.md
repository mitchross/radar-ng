...
**Data-flow rules.** All server data flows `react-query hook → zustand (useWeatherStore) → component`. Components never fetch. MMKV persists user prefs (server URL, theme, palette, extras — parsed with fallbacks on load, never cast) *and* the last manifest — `useManifestQuery` seeds react-query's `initialData` from MMKV with `initialDataUpdatedAt: 0`, so a cold start renders the radar instantly from cache and refetches immediately. That one query key (`["manifest", serverUrl]`) is shared by the radar tab, the home mini-map and Settings, so a single 30 s poll feeds all three; forecast, alerts, lightning, storms, and tropical each have their own hook and cadence. Every fetch carries a 10 s deadline chained to react-query's abort signal (`fetchWithTimeout`; Hermes has no `AbortSignal.timeout`). The root layout wires react-query's `focusManager` to `AppState`, so interval polling and the 750 ms playback tick pause while the app is backgrounded and stale queries refetch on return.
...
**Playback today: one source, remounted per frame.** A mounted MapLibre `RasterSource` cannot change its tile URL in place, so `RadarOverlay.tsx` mounts exactly one `RasterSource` keyed on `(palette, layer, frame.path)` and remounts it on every 750 ms tick (`TimelineBar.tsx`): unmount → request every visible tile → decode → GPU upload → paint, with `raster-fade-duration: 0`, so a frame is blank until its first tiles land. After the first loop the 24 h `immutable` tile cache keeps the network quiet, but the decode/upload cost is paid on every loop and on every scrub step. `WeatherLayerOverlay` and the home `RadarMiniMap` use the same remount pattern. Frame identity across manifest polls is the frame's `time`, not its index (`resnapFrameIndex`), so a poll that prunes the head does not move the paused frame.

**Planned: restore the 5-slot carousel.** Commit `b12012f` shipped `frontend/src/lib/radarCarousel.ts` (pure `assignSlots`/`clampWindow`, unit-tested) and a `WINDOW = 5` `RadarOverlay`: five raster sources always mounted, frame advance as an opacity swap instead of a remount, exactly one hidden remount per tick with ≈3 s of prefetch time before that slot is shown. It was lost in the `dc1af8c` redesign merge and is recoverable with `git show b12012f:frontend/src/lib/radarCarousel.ts`. An earlier 7-source preload crashed iOS with an `NSRangeException` in `[MLRNMapView insertReactSubview:atIndex:]` because the native child *count* churned; the carousel's constant child count is what avoids that — but it still changes the `<Map>` child count from 1 to 5, so it comes back behind the `WINDOW` flag (`WINDOW = 1` = today's behaviour) only after testing on a physical iPhone with alerts present, extras toggled, and layer/palette/zoom switches.
