...
describe("assignSlots", () => {
  ...
  it("never reassigns the slot that is about to become visible", () => {
    // The visible slot must be one that was already mounted (prefetching) on the
    // previous tick — otherwise the swap is a remount, not an opacity flip.
    for (let idx = 0; idx < 40; idx++) {
      const prev = assignSlots(idx, 0, 40, N);
      const cur = assignSlots(idx + 1, 0, 40, N);
      expect(prev.slots[cur.visibleSlot]).toBe(idx + 1);
    }
  });
  ...
  it("always fills every slot with a valid index inside the (widened) window", () => {
    for (let idx = 0; idx < 25; idx++) {
      for (const n of [1, 2, 3, 5, 7]) {
        const { slots, visibleSlot } = assignSlots(idx, 4, 20, n);
        expect(slots).toHaveLength(n);
        const lo = Math.min(4, idx);
        const hi = Math.max(20, idx);
        for (const f of slots) {
